...
class SettingGroupAdmin(admin.ModelAdmin):
    list_display = ['group_name', 'last_modified']
...
class SettingGroupAdmin(admin.ModelAdmin):
    ...
    def changeform_view(self, request, form_url='', extra_context=None):
        with transaction.atomic(using=router.db_for_write(self.model)):
            return self._changeform_view(request, form_url, extra_context)

    def _changeform_view(self, request, form_url, extra_context):
        model = self.model
        opts = model._meta
        ...
        adminForm = helpers.AdminForm(
            ...
            self.get_prepopulated_fields(request, obj) if add or self.has_change_permission(
                request, obj) else {},
            readonly_fields,
            model_admin=self)
    ...
    def get_changeform_initial_data(self, request):
        settings = get_manager('app_settings.ApplicationSetting').filter(
            group__group_name=self.model.name_from_class()
